  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedSamples = samples.map(s =>
      s.id === selectedSample.id
        ? {
            ...s,
            result: Number(resultData.result),
          ...
          <Table>
            ...
            <TableBody>
              {samples.map((sample) => {
                ...
                const compliance = sample.result && load ? getComplianceStatus(sample.result, load.fck) : null;

                return (
                  <TableRow key={sample.id}>
                    <TableCell>{sample.code}</TableCell>
                    <TableCell>{work?.name || 'N/A'}</TableCell>
                    <TableCell>{load?.loadNumber || 'N/A'}</TableCell>
                    <TableCell>{load?.fck} MPa</TableCell>
                    <TableCell>{sample.age} dias</TableCell>
                    <TableCell>{new Date(sample.moldingDate).toLocaleDateString('pt-BR')}</TableCell>
                    <TableCell>{new Date(sample.testDate).toLocaleDateString('pt-BR')}</TableCell>
                    <TableCell>
                      {sample.result ? (
                        <span className={compliance?.color}>
                          {sample.result.toFixed(2)} MPa
                        </span>
                      ) : (
                    ...
                    <TableCell className="text-right">
                      <Button
                        size="sm"
                        variant={sample.result ? 'outline' : 'default'}
                        onClick={() => handleRegisterResult(sample)}
                      >
                        <FlaskConical className="w-4 h-4 mr-2" />
                        {sample.result ? 'Editar' : 'Registrar'}
                      </Button>
                    </TableCell>
                  </TableRow>
                );
              ...
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    required
                  />
                  {selectedSample && loads.find(l => l.id === selectedSample.loadId) && (
                    <p className="text-sm text-gray-600">
                      FCK esperado: {loads.find(l => l.id === selectedSample.loadId)?.fck} MPa
                    </p>
                  )}
                </div>
                <div className="grid gap-2">
