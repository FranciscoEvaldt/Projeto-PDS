...
export function ReportsManager() {
  ...
  const handleDownloadDocx = async (
    ...
    try {
      ...
      const docx = generateConcreteTestReport(
        work,
        company,
        sheetLoads,
        sheetSamples
  ...
  const filteredLoads = loads.filter((load) => {
    const matchesWork =
      selectedWork === "all" || load.obra_id === Number(selectedWork);
  ...
  const getSamplesBySheetAndAge = (sheetLoads: typeof loads) => {
    const sheetSamples = samples.filter(
      (s) =>
        sheetLoads.some((load) => load.id === s.carga_id) &&
        s.idade_dias !== undefined
    );
                ...
                <div key={key} className="space-y-3">
                  <Card className="border-2 border-blue-200 bg-blue-50">
                    <CardContent className="pt-4 pb-4">
                      ...
                      <p className="text-sm text-gray-600 mb-2">
                        Planilha #{sheetLoads[0].numero_planilha} •{" "}
                        {work?.codigo} • {sheetLoads.length} carga(s)
                      </p>
                      ...
                      <div className="grid grid-cols-3 gap-x-6 text-sm">
                        ...
                        <div>
                          ...
                          <span className="ml-2 font-medium">
                            {sheetLoads[0].fornecedor_concreto || "-"}
                          </span>
                        </div>
                        <div>
                          <span className="text-gray-600">Data Moldagem:</span>
                          <span className="ml-2 font-medium">
                            {formatDateBR(sheetLoads[0].data_moldagem)}
                          </span>
                        </div>
                      </div>
                    </CardContent>
                  ...
                  <div className="ml-6 space-y-3">
                    {ages.map((age) => {
                      ...
                      const testedSamples = ageSamples.filter(
                        (s) => s.resistencia_mpa
                      );
                      const allTested =
                        ageSamples.length > 0 &&
                        ageSamples.length === testedSamples.length;
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                <div className="flex items-center gap-3 mb-2">
                                  ...
                                  >
                                    {testedSamples.length}/{ageSamples.length}{" "}
                                    ensaiadas
                                  </span>
                                </div>
                                ...
                                <div className="text-sm space-y-1">
                                  ...
                                  <div>
                                    <span className="text-gray-600">
                                      Data prevista de rompimento:
                                    </span>
                                    <span className="ml-2 font-medium">
                                      {formatDateBR(
                                        ageSamples[0].data_prevista_rompimento
                                      )}
                                    </span>
                                  </div>
                                  {allTested && (
