import { Document, Packer, Paragraph, TextRun, Table, TableRow, TableCell, WidthType, AlignmentType, BorderStyle, HeadingLevel, convertInchesToTwip, ShadingType } from 'docx';
...
export function generateConcreteTestReport(work: Work, company: Company, loads: Load[], samples: Sample[]) {
  const reportNumber = `${new Date().getFullYear()}/${loads[0]?.numero_planilha?.toString().padStart(2, '0') || '01'}`;
  const firstLoad = loads[0];
  const lastSample = samples
    .filter(s => s.data_prevista_rompimento) // Filtra amostras com data definida
    .sort((a, b) => 
      new Date(b.data_prevista_rompimento!).getTime() - new Date(a.data_prevista_rompimento!).getTime()
    )[0];
        ...
        new TableRow({
          children: [
            ...
            new TableCell({
              ...
              children: [
                new Paragraph({
                  children: [new TextRun({ text: 'Model Engenharia e Tecnologia Ltda', size: 16 })],
                  alignment: AlignmentType.RIGHT,
                }),
                new Paragraph({
                  children: [new TextRun({ text: 'Via Morro Azul, 450', size: 16 })],
                  alignment: AlignmentType.RIGHT,
                }),
                new Paragraph({
                  children: [new TextRun({ text: 'CEP: 95.780-000', size: 16 })],
                  alignment: AlignmentType.RIGHT,
                }),
                new Paragraph({
                  children: [new TextRun({ text: 'Três Cachoeiras (SC)', size: 16 })],
                  alignment: AlignmentType.RIGHT,
                }),
              ],
            }),
  ...
  sections.push(
    new Paragraph({
      children: [new TextRun({ text: '1. Dados do cliente', bold: true, size: 18 })],
      spacing: { before: 100, after: 120 },
    }),
    new Paragraph({
      children: [
        new TextRun({ text: 'Razão social: ', bold: true, size: 18 }),
        new TextRun({ text: company.nome || 'Não informado', color: 'CC0000', italics: true, size: 18 }),
      ],
      spacing: { after: 60 },
    }),
    new Paragraph({
      children: [
        ...
        new TextRun({ text: company.endereco || work.endereco || 'Não informado', color: 'CC0000', italics: true, size: 18 }),
      ],
      spacing: { after: 60 },
    }),
    new Paragraph({
      children: [
        ...
        new TextRun({ text: work.contrato || 'Não informado', color: 'CC0000', italics: true, size: 18 }),
      ],
      spacing: { after: 60 },
    }),
    new Paragraph({
      children: [
        ...
        new TextRun({ text: 'Cliente', color: 'CC0000', italics: true, size: 18 }),
      ],
      spacing: { after: 60 },
    })
  );
  ...
  sections.push(
    new Paragraph({
      children: [new TextRun({ text: '2. Responsáveis:', bold: true, size: 18 })],
      spacing: { before: 200, after: 120 },
    }),
    new Paragraph({
      children: [
        new TextRun({ text: 'Relatório de ensaio autorizado por: ', bold: true, size: 18 }),
        new TextRun({ text: 'Eng. Civil Felipe Model.', color: 'CC0000', italics: true, size: 18 }),
      ],
      spacing: { after: 60 },
    }),
    new Paragraph({
  ...
  sections.push(
    ...
    new Paragraph({
      children: [
        new TextRun({ text: 'Número(s) da(s) Amostra(s): ', bold: true, size: 18 }),
        new TextRun({ 
    ...
    new Paragraph({
      children: [
        ...
        new TextRun({ text: formatDate(firstLoad?.data_moldagem), color: 'CC0000', italics: true, size: 18 }),
      ],
      spacing: { after: 60 },
    }),
    new Paragraph({
      children: [
        new TextRun({ text: 'Período de Realização do Ensaio: ', bold: true, size: 18 }),
        new TextRun({ 
          text: `de ${formatDate(firstLoad?.data_moldagem)} até ${formatDate(lastSample?.data_prevista_rompimento)}`, 
          color: 'CC0000', 
          italics: true, 
          size: 18 
        }),
    ...
    new Paragraph({
      children: [
        new TextRun({ text: 'Realização das atividades: ', bold: true, size: 18 }),
        new TextRun({ 
  ...
  sections.push(
    new Paragraph({
      children: [
        ...
        new TextRun({ text: firstLoad.fornecedor_concreto || '-', color: 'CC0000', italics: true, size: 18 }),
      ],
      spacing: { after: 60 },
    }),
    new Paragraph({
    ...
    new Paragraph({
      children: [
        new TextRun({ text: 'Resistência característica (fck): ', bold: true, size: 18 }),
        new TextRun({ text: `${firstLoad.fck_mpa} MPa`, color: 'CC0000', italics: true, size: 18 }),
      ],
      spacing: { after: 60 },
    }),
    new Paragraph({
      children: [
        ...
        new TextRun({ text: `${firstLoad.slump_cm || '-'} mm`, color: 'CC0000', italics: true, size: 18 }),
      ],
      spacing: { after: 60 },
    }),
    new Paragraph({
  ...
  // TABELA PRINCIPAL - Resultados dos ensaios
  sections.push(
    new Paragraph({
      children: [
        new TextRun({ 
  ...
  // Cria a tabela com todas as amostras
  const tableRows = [
    // Cabeçalho
    new TableRow({
      tableHeader: true,
    ...
    tableRows.push(
      new TableRow({
        children: [
          ...
          new TableCell({
            children: [new Paragraph({ 
              text: sample.resistencia_mpa ? Number(sample.resistencia_mpa).toFixed(1) : '-', 
              alignment: AlignmentType.CENTER,
              spacing: { after: 0, before: 0 } 
            })],
          }),
  ...
  // ===== RESISTÊNCIAS MÉDIAS POR IDADE =====
  Object.keys(samplesByAge).sort((a, b) => Number(a) - Number(b)).forEach(ageStr => {
    const age = Number(ageStr);
    const ageSamples = samplesByAge[age];
    const avgResistance = ageSamples.reduce((sum: number, s: any) => sum + (Number(s.resistencia_mpa) || 0), 0) / ageSamples.length;

    sections.push(
      new Paragraph({
        children: [
          new TextRun({ text: `Resistência média aos ${age} dias: `, bold: true, size: 18 }),
          new TextRun({ text: `${avgResistance.toFixed(1)} MPa`, bold: true, color: '00AA00', size: 18 }),
        ],
        spacing: { after: 100 },
      })
    );
  ...
  sections.push(
    new Paragraph({
      children: [new TextRun({ text: '6. Equipamentos:', bold: true, size: 18 })],
      spacing: { before: 300, after: 120 },
    }),
    new Paragraph({
      children: [
        new TextRun({ text: 'Prensa hidráulica: ', bold: true, size: 18 }),
        new TextRun({ text: 'EMIC - modelo DL 200000, cap. 2000 kN', size: 18 }),
      ],
      spacing: { after: 60 },
    }),
    new Paragraph({
  ...
  sections.push(
    new Paragraph({
      children: [new TextRun({ text: '7. Observações:', bold: true, size: 18 })],
      spacing: { before: 300, after: 120 },
    }),
    new Paragraph({
      children: [
  ...
  sections.push(
    ...
    new Paragraph({
      children: [new TextRun({ text: 'Eng. Civil Felipe Model', bold: true, size: 18 })],
      alignment: AlignmentType.CENTER,
      spacing: { after: 50 },
    }),
    new Paragraph({
  ...
  sections.push(
    ...
    new Paragraph({
      children: [
        new TextRun({ text: 'Model Engenharia e Tecnologia Ltda - Via Morro Azul, 450 - Três Cachoeiras/SC', size: 16 }),
      ],
      alignment: AlignmentType.CENTER,
      spacing: { after: 50 },
    }),
    new Paragraph({
      children: [
        new TextRun({ text: 'Telefone: (51) 3546-1234 | Email: contato@modelengenharia.com.br', size: 16, italics: true }),
      ],
      alignment: AlignmentType.CENTER,
    })
  );
