...
export function generateConcreteTestReport(
  ...
  const samplesByAge = samples.reduce(
    (acc: Record<number, Sample[]>, sample) => {
  ...
  sections.push(
    ...
    new Paragraph({
      children: [
        ...
        new TextRun({
          text:
            work.empresa_id !== null && work.empresa_id !== undefined
              ? String(work.empresa_id)
              : "-",
          color: "CC0000",
          italics: true,
          size: 18,
  ...
  sections.push(
    ...
    new Paragraph({
      children: [
        ...
        new TextRun({
          text: samples.map((s: Sample) => s.numero_laboratorio).join(", "),
          color: "CC0000",
          italics: true,
          size: 18,
        }),
  ...
  samples.forEach((sample: Sample) => {
    const load = loads.find((l: Load) => l.id === sample.carga_id);
    tableRows.push(
      new TableRow({
        children: [
          new TableCell({
          ...
          new TableCell({
            children: [
              new Paragraph({
                text: String(sample.numero_laboratorio ?? "-"),
                alignment: AlignmentType.CENTER,
                spacing: { after: 0, before: 0 },
              }),
            ],
          ...
          new TableCell({
            children: [
              new Paragraph({
                text: sample.resistencia_mpa?.toFixed(1) || "-",
                alignment: AlignmentType.CENTER,
                spacing: { after: 0, before: 0 },
              }),
            ],
  ...
  Object.keys(samplesByAge)
    ...
    .forEach((ageStr) => {
      ...
      const avgResistance =
        ageSamples.reduce(
          (sum: number, s: Sample) => sum + (s.resistencia_mpa || 0),
          0
        ) / ageSamples.length;
