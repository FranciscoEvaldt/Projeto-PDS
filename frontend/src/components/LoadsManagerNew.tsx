...
import { Plus, Truck, Package, FolderOpen, FileText, Eye, Pencil, Trash2, Calendar, Box, Building2, MapPin, ClipboardList } from 'lucide-react';
...
export function LoadsManagerNew() {
  const { companies, works, loads, samples, createLoad, updateLoad, deleteLoad, createSamplesBulk } = useData();
  ...
  const generateSamples = async (loadId: number, moldingDate: string, samplesConfig: { age: number; quantity: number }[]) => {
    const newSamples: Omit<Sample, 'id' | 'created_at' | 'updated_at' | 'created_by' | 'updated_by'>[] = [];
    
    let sequencia = 1;
    let nextLabNumber = Math.max(0, ...samples.map(s => s.numero_laboratorio)) + 1;
  ...
  const getTotalSamples = () => {
    return (parseInt(formData.samples7d) || 0) + 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Calcular o próximo número de planilha para esta obra
      const numeroPlanilha = getNextPlanilhaNumber(loads, Number(selectedWorkId));
      ...
      const promises = formData.loads.map(async (loadData) => {
        ...
        const loadPayload: LoadFormData = {
          ...
          volume_m3: loadData.volume_m3 ? Number(loadData.volume_m3) : undefined,
        ...
        await generateSamples(newLoad.id, loadData.data_moldagem, samplesConfig);
      ...
      toast.success(`Planilha #${numeroPlanilha} criada com ${formData.loads.length} carga(s) para ${obra?.nome}!`);
    } catch (error: unknown) {
  ...
  const handleDelete = async (loadId: number) => {
    if (window.confirm('Tem certeza que deseja excluir esta carga? Todas as amostras relacionadas serão excluídas.')) {
      try {
  ...
  // Agrupar cargas por obra e planilha
  const groupedData = () => {
    ...
    loads.forEach(load => {
      ...
      const planilhaKey = `${load.obra_id}_${load.numero_planilha}`;
      let planilha = grouped[obraKey].planilhas.find(
        p => p.obra_id === load.obra_id && p.numero_planilha === load.numero_planilha
      );
              ...
              <div className="space-y-4">
                ...
                <Accordion type="multiple" className="space-y-2">
                  {formData.loads.map((load, index) => (
                    <AccordionItem key={index} value={`load-${index}`} className="border rounded-lg">
                      ...
                      <AccordionContent className="px-4 pb-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <Input
                              value={load.caminhao}
                              onChange={(e) => updateLoadData(index, 'caminhao', e.target.value)}
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => updateLoadData(index, 'nota_fiscal', e.target.value)}
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => updateLoadData(index, 'data_moldagem', e.target.value)}
                              required
                            />
                          </div>
                          <div>
                            ...
                            <Input
                              type="number"
                              value={load.fck_mpa}
                              onChange={(e) => updateLoadData(index, 'fck_mpa', e.target.value)}
                              placeholder="Ex: 30"
                            />
                          </div>
                          <div>
                            ...
                            <Input
                              value={load.slump_cm}
                              onChange={(e) => updateLoadData(index, 'slump_cm', e.target.value)}
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => updateLoadData(index, 'volume_m3', e.target.value)}
                              placeholder="Ex: 8.50"
                            />
                          </div>
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => updateLoadData(index, 'pavimento', e.target.value)}
                          ...
                          <div>
                            ...
                            <Input
                              value={load.peca}
                              onChange={(e) => updateLoadData(index, 'peca', e.target.value)}
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => updateLoadData(index, 'fornecedor_concreto', e.target.value)}
                          ...
                          <div className="col-span-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => updateLoadData(index, 'observacoes', e.target.value)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingLoad({ ...editingLoad, caminhao: e.target.value })}
                    placeholder="Ex: CAM-01"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingLoad({ ...editingLoad, nota_fiscal: e.target.value })}
                    placeholder="Ex: NF-12345"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingLoad({ ...editingLoad, data_moldagem: e.target.value })}
                    required
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingLoad({ ...editingLoad, fck_mpa: e.target.value ? Number(e.target.value) : undefined })}
                    placeholder="Ex: 30"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingLoad({ ...editingLoad, slump_cm: e.target.value })}
                    placeholder="Ex: 100±20"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingLoad({ ...editingLoad, volume_m3: e.target.value ? Number(e.target.value) : undefined })}
                    placeholder="Ex: 8.50"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingLoad({ ...editingLoad, pavimento: e.target.value })}
                    placeholder="Ex: Térreo"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingLoad({ ...editingLoad, peca: e.target.value })}
                    placeholder="Ex: Pilares P1-P10"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingLoad({ ...editingLoad, fornecedor_concreto: e.target.value })}
                    placeholder="Ex: Concreteira Sul"
                  />
                </div>
                <div className="col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingLoad({ ...editingLoad, observacoes: e.target.value })}
                    placeholder="Observações sobre a carga"
                  />
                </div>
              </div>
      ...
      <Dialog open={isDetailsDialogOpen} onOpenChange={setIsDetailsDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {viewingLoad && (() => {
            ...
            const empresa = obra ? companies.find(c => c.id === obra.empresa_id) : null;
            const loadSamples = samples.filter(s => s.carga_id === viewingLoad.id);
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-4 p-4 bg-muted/30 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="font-medium">#{viewingLoad.numero_planilha}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-4 p-4 bg-muted/30 rounded-lg">
                    <div>
                      ...
                      <p className="font-medium">{viewingLoad.caminhao || '-'}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Nota Fiscal</p>
                      <p className="font-medium">{viewingLoad.nota_fiscal || '-'}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Data de Moldagem</p>
                    ...
                    <div>
                      ...
                      <p className="font-medium">{viewingLoad.fck_mpa ? `${viewingLoad.fck_mpa} MPa` : '-'}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Slump</p>
                      <p className="font-medium">{viewingLoad.slump_cm || '-'}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Volume</p>
                      <p className="font-medium">{viewingLoad.volume_m3 ? `${viewingLoad.volume_m3} m³` : '-'}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Pavimento</p>
                      <p className="font-medium">{viewingLoad.pavimento || '-'}</p>
                    </div>
                    <div>
                    ...
                    {viewingLoad.fornecedor_concreto && (
                      <div className="col-span-2">
                        <p className="text-sm text-muted-foreground">Fornecedor de Concreto</p>
                        <p className="font-medium">{viewingLoad.fornecedor_concreto}</p>
                      </div>
                    )}
                    {viewingLoad.observacoes && (
                      <div className="col-span-2">
                        <p className="text-sm text-muted-foreground">Observações</p>
                        <p className="font-medium">{viewingLoad.observacoes}</p>
                      </div>
                    )}
                  </div>
                </div>
                    ...
                    {loadSamples.length > 0 ? (
                      <div className="grid gap-2">
                        {loadSamples.map(sample => (
                          <div key={sample.id} className="p-3 border rounded-lg bg-background">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm text-muted-foreground">
                                  Previsto: {formatDateBR(sample.data_prevista_rompimento)}
                                </span>
                              </div>
                              <Badge 
                                variant={
                                  sample.status === 'tested' ? 'default' : 
                                  sample.status === 'pending' ? 'secondary' : 
                                  'destructive'
                                }
                              >
                                {sample.status === 'tested' ? 'Testado' : 
                                 sample.status === 'pending' ? 'Pendente' : 
                                 'Reprovado'}
                              </Badge>
                            </div>
                            {sample.resistencia_mpa && (
                              <div className="mt-2 text-sm">
                                ...
                                <span className="font-medium">{Number(sample.resistencia_mpa).toFixed(2)} MPa</span>
                              </div>
                            )}
                          </div>
                        ))}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-4 text-sm">
                  <Badge variant="secondary">
                    {obra.total_cargas} {obra.total_cargas === 1 ? 'carga' : 'cargas'}
                  </Badge>
                  <Badge variant="outline">
                    {obra.total_amostras} {obra.total_amostras === 1 ? 'amostra' : 'amostras'}
                  </Badge>
                </div>
              </div>
            </CardHeader>
                    ...
                    <AccordionTrigger className="px-4 hover:no-underline">
                      <div className="flex items-center justify-between w-full pr-4">
                        ...
                        <div className="flex gap-2">
                          <Badge variant="secondary">
                            {planilha.cargas.length} {planilha.cargas.length === 1 ? 'carga' : 'cargas'}
                          </Badge>
                          <Badge variant="outline">
                            {planilha.total_amostras} {planilha.total_amostras === 1 ? 'amostra' : 'amostras'}
                          </Badge>
                        </div>
                      </div>
                    </AccordionTrigger>
                            ...
                            >
                              <div className="space-y-2 flex-1">
                                ...
                                <div className="grid grid-cols-3 gap-x-4 gap-y-1 text-sm text-muted-foreground">
                                  <div>
                                    FCK: {load.fck_mpa || '-'} MPa
                                  </div>
                                  <div>
                                    Volume: {load.volume_m3 || '-'} m³
                                  </div>
                                  <div>
                                    {loadSamples.length} {loadSamples.length === 1 ? 'amostra' : 'amostras'}
                                  </div>
                                </div>
